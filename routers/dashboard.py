...
def calculate_sentiment_trend(db: Session, user_id: uuid.UUID, today: date) -> float:
    ...
    last_7 = last_7_avg if last_7_avg is not None else 0.0
    prev_7 = prev_7_avg if prev_7_avg is not None else 0.0
...
def get_dashboard(user_id: str, db: Session = Depends(database.get_db)):
    ...
    # All individual log points for last 7 days
    my_logs_7d = db.query(
    ...
    my_scores = [{"date": row.logged_at.isoformat(), "score": row.score} for row in my_logs_7d]
    ...
    if partner_id:
        ...
        partner_scores = [{"date": row.logged_at.isoformat(), "score": row.score} for row in partner_logs_7d]
        ...
        if latest_risk:
            ...
            dashboard_data["features_snapshot"] = latest_risk.features_snapshot
            ...
            if tier:
                dashboard_data["suggestion"] = tier_mapping[tier]
